...
import { ftGetTokenMetadata, ftGetBalance } from "./token";
import { TokenMetadata } from "./ft-contract";
import { RefFiFunctionCallOptions, Transaction } from "@/interfaces/wallet";
import { executeMultipleTransactions } from "@/utils/near";
import { ONE_YOCTO_NEAR, REF_FARM_BOOST_CONTRACT_ID } from "@/utils/contract";
      ...
      functionCalls: [
        {
          ...
          args: {
            ...
            memo: "",
            msg: getAccountId() + ":" + Zero64 + address.substring(2),
          },
          gas: depositGas,
...
export const useTriTokenIdsOnRef = (stopOn?: boolean) => {
  ...
  const [triTokenIds, setTriTokenIds] = useState<any>(null);
...
export const getAllTriPools = async (pair?: [string, string]) => {
  const allSupportPairs: any = getAuroraConfig().Pairs;
  const auroraTokens = defaultTokenList.tokens;
  const address = auroraAddr(getAccountId());
  const symbolToAddress: any = auroraTokens.reduce((pre, cur, i) => {
    return {
      ...pre,
      [cur.symbol]: cur.address,
  ...
  const pairAddresses = Object.keys(allSupportPairs)
    ...
    .filter((p) => {
      const showPair = (pair as any).map((p: any) => {
        return p;
      });
...
export const hasTriPools = (pair?: [string, string] | any) => {
  const allSupportPairs: any = getAuroraConfig().Pairs;
  const auroraTokens = defaultTokenList.tokens;
  const address = auroraAddr(getAccountId());
  const symbolToAddress: any = auroraTokens.reduce((pre, cur, i) => {
...
export const auroraSwapTransactions = async ({
  ...
  tokenIn_id: string;
  tokenOut_id: string;
  swapTodos: EstimateSwapView[] | any;
  readableAmountIn: string;
  readableAmountOut?: string | any;
  decimalIn: number;
  decimalOut: number;
